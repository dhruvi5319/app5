...
| Resume Later | Opens app on laptop that evening | Browser / F4 (localStorage restore) | "Let me check what I captured earlier today" | Expecting continuity | If tasks saved on mobile are missing on laptop (different browser or device): localStorage is browser-scoped; cross-device sync is out of scope for v1 | A static footer note "Tasks are saved locally in this browser only." sets expectations without alarming the user; cross-device sync is a v2 consideration |
